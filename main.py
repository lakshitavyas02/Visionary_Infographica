...
# adding padding to the top of container
st.markdown('<style> div.block-container{padding-top:1rem;} </style>', unsafe_allow_html=True)

#using this function, a user can upload a file
fl = st.file_uploader(" :file_folder: Upload a file", type = (['csv', 'txt', 'xlsx', 'xls']))
if fl is not None:
    filename = fl.name
    st.write(filename)
    df = pd.read_csv(filename, encoding= "ISO-8859-1") #reading the uploaded file

# if no file is uploaded, then we will read the file which we have downloaded already to show our program features
else:
    os.chdir(r"C:\Users\VARTIKA\Desktop\streamlit_app")
    df = pd.read_csv('Superstore.csv', encoding= "ISO-8859-1")

# creating date picker, so that user can define the duration of data to be analyzed
